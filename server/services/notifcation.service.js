...
export const NotificationService = {
    ...
    async getNotifications(context) {
        ...
        return await NotificationRepo.findByUser(user.id).orElseThrow(new Error("No notifications found"));
    },

    async getNotification(id, context) {
        const { user } = context;

        const notification = await NotificationRepo.findById(id).orElseThrow(new Error("Notification not found"));
    ...
    async markAllAsRead(context) {
        ...
        const notifications = await NotificationRepo.findByUser(user.id).orElseThrow(new Error("No notifications found"));
    ...
    async deleteNotification(id, context) {
        ...
        if (notification.user.toString() !== user.id && user.role !== "SUPER_ADMIN") {
